...
class CalendarAnswerController extends Controller
{
    ...
    public function index()
    {
        if($this->isTeacher())
        {
            $content = StudentAnswerDegree::with(['student:id,name', 'calendar:id,title'])->whereHas('calendar', function($q){
                return $q->where('teacher_id', $this->userId());
            });
        }
        else
        {
    ...
    public function edit(StudentAnswerDegree $calendarAnswer)
    {
        ...
        if($this->isTeacher())
        {
            if($content->calendar?->teacher_id != $this->userId())
            {
                return abort(404);
            }
        }
    ...
    public function update(UpdateStudentAnswerDegreeRequest $request, StudentAnswerDegree $calendarAnswer)
    {
        ...
        $student_final_degree = array_sum(Arr::except($data, ['question_degree','calendar_final_calendar_degree', 'sentimental_side_degree', 'performance_side_degree', 'knowledge_side_degree']));
