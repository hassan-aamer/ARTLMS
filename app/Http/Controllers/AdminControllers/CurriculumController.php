...
class CurriculumController extends Controller
{
    ...
    public function index()
    {
        if($this->isTeacher())
        {
            $content =  Curriculum::with('level')->whereIn('level_id', $this->teacher_levels())->orderBy('sort', 'asc')->paginate($this->paginate);
        }
        else
        {
    ...
    public function edit(Curriculum $curriculum)
    {
        ...
        return view('admin_dashboard.curriculums.edit', compact('content','levels', 'terms', 'skills', 'arrayCourseSkills'));
    }
    ...
    public function update(CurriculumRequest $request, Curriculum $curriculum)
    {
        ...
        try {
            ...
            DB::rollBack();
            toastr()->error($this->updateMsg, 'لم تتم العملية ، تأكد من صحة البيانات', ['timeOut' => 5000]);
            return redirect()->back();
        }
    }
